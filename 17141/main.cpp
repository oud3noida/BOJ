...
constexpr int DX[4] = {0, 0, -1, 1},
              DY[4] = {-1, 1, 0, 0};
int n, num_viruses, num_virus_candidates, num_blanks;
vector<vector<int>> board;
vector<Position> virus_candidates;
...
int time_to_full() {
    ...
    for (int index : virus_index) {
        Position virus = virus_candidates[index];
        q.emplace(virus, 0);
    ...
    if (num_filled_blanks == num_blanks + num_virus_candidates) {
        return cur_time;
    }
...
int min_time_to_full(int cur, int prev_index) {
    ...
    int ret = INF;
    for (int index=prev_index+1; index<num_virus_candidates; ++index) {
...
int main()
{
    ...
    num_blanks = 0;
    num_virus_candidates = 0;
    for (int y=0; y<n; ++y) {
        for (int x=0; x<n; ++x) {
            cin >> board[y][x];
            if (board[y][x] == BLANK) ++num_blanks;
            if (board[y][x] == VIRUS) {
                virus_candidates.emplace_back(x, y);
                ++num_virus_candidates;
            }
        }
    }
